...
const PlaceCard = ({ place, isCompact = false }) => {
    ...
    const handleToggleFavorite = async (e) => {
        ...
        try {
            ...
            if (!isFavorite) {
                updatedUser.favorites = [...(updatedUser.favorites || []), place._id];
            } else {
                updatedUser.favorites = updatedUser.favorites.filter(id => id !== place._id);
            }
        ...
        return (
            <Card className="flex flex-col h-full group">
                <div className="relative h-48 overflow-hidden">
                    <img
                        src={place.images?.[0] || 'https://images.unsplash.com/photo-1596402184320-417d717867cd?auto=format&fit=crop&w=400&q=80'}
                        alt={place.name}
                        loading="lazy"
                ...
                <CardContent className="flex-1 flex flex-col p-16">
                    <h3 className="text-body font-bold text-heritage-brown dark:text-heritage-gold mb-4 line-clamp-1">{place.name}</h3>
                    <div className="flex items-center gap-4 text-small text-heritage-gold font-bold mb-12">
                        <Star size={14} fill="currentColor" />
                        <span>{place.rating}</span>
                    </div>
                    <Link to={`/place/${place._id}`} className="mt-auto">
    ...
    return (
        <Card className="flex flex-col h-full">
            <div className="relative h-64 overflow-hidden">
                <Link to={`/place/${place._id}`} className="block h-full">
                    <img
                        src={place.images?.[0] || 'https://images.unsplash.com/photo-1596402184320-417d717867cd?auto=format&fit=crop&w=800&q=80'}
                        alt={place.name}
                        loading="lazy"
                ...
                <div className="absolute top-16 left-16">
                    <Badge variant="gold" className="backdrop-blur-md bg-heritage-gold/80 text-white border-none uppercase tracking-widest">
                        {place.category}
                    </Badge>
                </div>
                <button
